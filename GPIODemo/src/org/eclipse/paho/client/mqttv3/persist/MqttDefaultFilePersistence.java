...
//		dataDir = new File(directory);
		...
		dataDir = FileSystems.getDefault().getPath(directory);
	}
...
//			if (!dataDir.mkdirs()) {
		...
		synchronized (this) {
			if (clientDir == null) {
				String key = keyBuffer.toString();
//				clientDir = new File(dataDir, key);
				clientDir = FileSystems.getDefault().getPath(directory,key);
//				if (!clientDir.exists()) {
...
//					clientDir.mkdir();
	...
	public void remove(String key) throws MqttPersistenceException {
		checkIsOpen();
//		File file = new File(clientDir, key+MESSAGE_FILE_EXTENSION);
		Path file = FileSystems.getDefault().getPath(clientDir,key+MESSAGE_FILE_EXTENSION);
...
/*	private void restoreBackups(File dir) throws MqttPersistenceException {
		File[] files = dir.listFiles(new FileFilter() {
			public boolean accept(File f) {
				return f.getName().endsWith(MESSAGE_BACKUP_FILE_EXTENSION);  //测试并实现File.endWith函数
			}
		});
		if (files == null) {
			throw new MqttPersistenceException();
	...
	private void restoreBackups(Path dir) throws MqttPersistenceException {
		/*File[] files = dir.listFiles(new FileFilter() {
			public boolean accept(File f) {
				return f.getName().endsWith(MESSAGE_BACKUP_FILE_EXTENSION);
			}
		});*/
		ArrayList<Path> paths = listFiles(dir,MESSAGE_BACKUP_FILE_EXTENSION);
		if (paths == null) {
			throw new MqttPersistenceException();
		}
	...
	private ArrayList<Path> listFiles(Path dir,String backup){
		Iterable<Path> p=dir.getFileSystem().getRootDirectories();
		...
		while(iter!=null){
			//Path tmp = (Path)iter;
		///////////////////////////////////////
			if(tmp.endsWith(backup))   //Path的endWith和File的endWith是否相同？测试并重新实现File.endWith函数
		////////////////////////////////////////////////
				list.add(tmp);
			if(iter.hasNext())
				tmp = iter.next();
