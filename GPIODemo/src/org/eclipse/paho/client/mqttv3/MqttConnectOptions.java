...
//		try {
		//	URI vURI = new URI(srvURI);
			String URI_tmp = new String(srvURI);
			int tmp = URI_tmp.indexOf("://");
			int pos = URI_tmp.indexOf("/",tmp+3);
			if (pos!=-1) {
				throw new IllegalArgumentException(srvURI);
			}
			if (URI_tmp.startsWith("tcp://", 0))
				return URI_TYPE_TCP;
			else if (URI_tmp.startsWith("ssl://", 0)) {
				return URI_TYPE_SSL;
			}
			else if (URI_tmp.startsWith("local://", 0)) {
